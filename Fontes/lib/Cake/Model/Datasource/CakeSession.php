 ...
 */
	public static function init($base = null) {
		...
		if(Configure::read('Database.installed') === 'true' ){
			self::$model = ClassRegistry::init('Configuracao');
			ClassRegistry::removeObject('Configuracao');
		}
 ...
 */
	protected static function _defaultConfig($name) {
		...
		if( self::$model ){
			$result = self::$model->find('first', array('Configuracoes.id', 'Configuracoes.tempo_sessao') );
		}
		...
		if($result){
			$defaults = array(
				'php' => array(
					'cookie' => 'CAKEPHP',
					'timeout' => $result['Configuracao']['tempo_sessao'],
					'ini' => array(
