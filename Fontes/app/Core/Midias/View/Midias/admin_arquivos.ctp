<?php 
	...
	if($tipo_conteudo == 'noticia') {
		...
		$this->Html->addCrumb("{$acao} de Notícia - Documentos",  array('admin' => true, 'tipo_conteudo' => $tipo_conteudo, 'id_conteudo' => $id_conteudo, 'controller' => 'midias' ,'action' => 'arquivos'));
	} else {
		$this->Html->addCrumb('Páginas',  array('plugin' => 'paginas', 'controller' => 'paginas', 'action' => 'index'));
		$this->Html->addCrumb("{$acao} de Página - Documentos",  array('admin' => true, 'tipo_conteudo' => $tipo_conteudo, 'id_conteudo' => $id_conteudo, 'controller' => 'midias' ,'action' => 'arquivos'));
	}
			...
			<li class="<?php echo ($midia['Midia']['ativa']) ? 'ativo' : 'inativo';?>">
				<div class="preview">
          			<ul class="opcoesMidia">
          				...
          				<?php if ($k != 0) { ?>
	          				<li class="ant"><?php echo $this->Html->link('mover "'.$midia['Midia']['titulo'].'" para a posição anterior', array('admin' => true, 'tipo_conteudo' => $tipo_conteudo, 'id_conteudo' => $id_conteudo, 'controller' => 'midias_conteudos', 'action' => 'move_up', 'id_midia' => $midia['MidiasConteudo']['id']), array('class' => 'move_up')); ?></li>
						<?php } ?>	

						<?php if ($k != count($midias) - 1) { ?>
							<li class="prox"><?php echo $this->Html->link('mover "'.$midia['Midia']['titulo'].'" para a próxima posição', array('admin' => true, 'tipo_conteudo' => $tipo_conteudo, 'id_conteudo' => $id_conteudo, 'controller' => 'midias_conteudos', 'action' => 'move_down', 'id_midia' => $midia['MidiasConteudo']['id']), array('class' => 'move_down'));?></li>
						<?php } ?>
					</ul>

					<?php echo $this->Midias->thumb($midia['Midia']['arquivo'], $midia['Midia']['tipo_id']); ?>

					<p><strong>Título:</strong> <?php echo $midia['Midia']['titulo']; ?></p>
		...
		$(document).ready(function() {
			...
			$('#avancar').on('click', function(e) {
	          ...
	          if (numDocsSemDescricao > 0 ) {
	            ...
	            return confirm(strExiste + numDocsSemDescricao + strDoc + 'sem descrição, deseja prosseguir mesmo assim ?');
	          } 
	...
	echo '<div class="form">';
		 echo $this->Form->create('Midia', array('type' => 'file'));
		 	 echo '<fieldset id="acaoForm">';
				...
				echo $this->Form->input('descartar', array('type' => 'submit', 'label' => false, 'value' => 'Descartar', 'class' => 'controle', 'onclick' => "return confirm('Você tem certeza que deseja deletar esta {$nomeConteudo} ?');"));
