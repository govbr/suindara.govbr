    ...
    if( isset($this->request['pass'][2]) && !empty($this->request['pass'][2]) ){
        ...
        if(!$noticia){
          throw new NotFoundException('Noticia não encontrada');
            ...
            if(!$categoriaPai){
              if($categoriaNome->identificador != 'manuais'){
                throw new NotFoundException('Categoria não é manuais'); 
              }
            }else{
              if($categoriaPai->identificador != 'manuais'){
                throw new NotFoundException('Categoria não é filho de manuais');    
              }
            }
        }
    }else{
        ...
        <div class="container cinterno">
          ...
          <div class="row"> 
            <article class="span12">
              <?php 
                $this->Html->addCrumb(($categoriaPai) ? $categoriaPai->titulo : $categoriaNome->titulo, ($categoriaPai) ? $categoriaPai->createPath('manuais') : $categoriaNome->createPath('manuais') );
                $this->Html->addCrumb($categoriaNome->titulo, $categoriaNome->createPath('manuais', 'listar') );
                $this->Html->addCrumb($noticia->titulo);
              ?>

              <?php 
                if(strlen($noticia->titulo) < 80){
                  echo $this->element('breadcrumb');
                }else{
                  echo $this->element('breadcrumb', array('class' => 'twolines'));
                }
              ?>
